resource "aws_security_group" "workers" {
  name        = format("%s-workers", var.cluster_full_name)
  description = format("Security group for all nodes in the %s cluster", var.cluster_full_name)
  vpc_id      = var.vpc_id
  ...
  tags = merge(
    var.common_tags,
    {
      Name                                                      = format("%s-cluster-sg", var.cluster_full_name)
      format("kubernetes.io/cluster/%s", var.cluster_full_name) = "owned"
    },
  )
}
